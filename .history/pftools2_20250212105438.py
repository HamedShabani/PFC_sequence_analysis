...
def get_rate_all(data, neuron_tspidx, occ_gau, yedges, sigma_yidx, expanded_all_burst_tidxs, 
    ...
    This function estimates the rate maps (place fields) for multiple neurons by processing their 
    spike time indices. For each neuron, it first excludes spikes occurring near burst events, 
    optionally shuffles the spike times for control analyses, and then computes a smoothed firing 
    rate map based on the animal's y positions using Gaussian smoothing. The function returns the 
    rate maps, spike counts per y-bin, and the occupancy (or probability distribution) over the 
    y-axis bins.

    Parameters
    ----------
    data : dict
        Dictionary containing trial or behavioral information. For example, it should include a 
        key such as 'trial_idx_mask' which is used by the `spike_shuffle` function if spike shuffling 
        is enabled.
    neuron_tspidx : list or array-like
        A list (or array) where each element contains the spike time indices for a given neuron.
    occ_gau : array-like
        The occupancy vector (or Gaussian-smoothed occupancy) used in the rate map estimation.
    yedges : array-like
        Array defining the edges of the bins along the y-axis.
    sigma_yidx : float or int
        Smoothing parameter (sigma) for the y-axis index that controls the Gaussian smoothing.
    expanded_all_burst_tidxs : array-like
        Indices representing the time points of burst events. Spike times occurring near these 
        indices are excluded from the rate map computation.
    num_neurons : int
        Total number of neurons to process.
    y_ax : array-like
        The y-axis bins or centers used to store the computed rate maps. Its length defines the 
        number of bins.
    y : array-like
        The animal's y positions corresponding to each time point; used to assign spike positions.
    shuffle_spikes : bool, optional
        If True, the spike times for each neuron are circularly shuffled using the 
        `spike_shuffle` function to generate randomized rate maps (useful for control analyses).
        Default is False.
    ...
    r_all : ndarray
        ...
        for each neuron.
    counts_all : ndarray
        A 2D NumPy array of shape (num_neurons, len(y_ax)) containing the spike counts for each 
        neuron in each y-bin.
    p_all : ndarray
        A 1D NumPy array representing the occupancy (or probability distribution) across the y-bins, 
        as computed by the `GetRate` function.

    Notes
    -----
    This function depends on several external functions:
      - `exclude_idx`: Excludes spike times that occur near burst events.
      - `spike_shuffle`: (Optional) Circularly shuffles spike times for control analyses.
      - `GetRate`: Computes the smoothed firing rate map given spike positions and occupancy data.
    ...
    # Initialize arrays for storing spike counts and firing rates.
    ...
    for nid in range(num_neurons):
        ...
        # Optionally shuffle spike times to generate randomized rate maps.
        if shuffle_spikes:
            masked_tspidx, spk = spike_shuffle(data, masked_tspidx, None)

        # Get the animal's y positions corresponding to the neuron's (possibly shuffled) spike times.
        neuron_ysp = y[masked_tspidx]

        # Compute the smoothed firing rate map using the provided occupancy and smoothing parameters.
        rate, counts_ysp_gau, P_x = GetRate(neuron_ysp, occ_gau, yedges, sigma_yidx)

        # Store the computed firing rate map and spike count map.
        r_all[nid, :] = rate
        counts_all[nid, :] = counts_ysp_gau
        p_all = P_x  # Assuming occupancy is the same across neurons.
