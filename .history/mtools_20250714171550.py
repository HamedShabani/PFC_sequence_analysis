...
def merge_clusters2(temp_info, nrm, seqs, bursts, plot_figure, min_ratio, z_thr):
    """
    Merge similar clusters based on pairwise similarity metrics computed on the templates.
    
    This function uses the output structure (temp_info) produced by the `templates` function,
    along with normalization parameters (nrm), sequences (seqs), and burst data (bursts), to
    merge clusters that are highly similar. It first flags and replaces clusters marked as 
    "bad" (stored in temp_info['exclude']) with a value of -1. Then, it computes similarity 
    matrices using the `allmot` function on the current templates, finds the pair of clusters with 
    the highest similarity (based on a z-score and a binary constraint), and if the similarity 
    exceeds a threshold (`z_thr`), merges the two clusters. After merging, it recomputes the cluster 
    templates (using `templates`) and similarity metrics.
    
    Parameters
    ----------
    temp_info : dict
        Dictionary output from the `templates` function. Expected keys include:
          - 'ids_clust': cluster IDs for each sequence (stored as a list/array)
          - 'exclude': indices of clusters to be excluded
          - 'template': list of template sequences for each cluster
    nrm : array_like
        Normalization parameters (e.g., reference values for z-score computation).
    seqs : array_like
        The original list/array of sequences (e.g., cell activation orders) used to generate templates.
    bursts : array_like
        Burst data corresponding to the sequences.
    plot_figure : bool
        If True, displays figures of the similarity matrices before and after merging.
    min_ratio : float
        Minimum ratio criteria passed to the `templates` function to decide on cluster quality.
    z_thr : float
        Threshold for the z-score; clusters with similarity z-scores above this threshold are considered for merging.
    
    Returns
    -------
    temp_info2 : dict
        Updated temp_info dictionary after cluster merging.
    mask_badcluster : np.ndarray
        Boolean mask indicating which clusters were considered "bad" (and set to -1).
    """
    # Identify bad clusters (those whose IDs match the "exclude" indices).
    # Note: [temp_info['exclude']][0] is used to obtain the first element of the exclusion list.
    mask_badcluster = (temp_info['ids_clust'][0] == [temp_info['exclude']][0])
    
    # Replace bad clusters with -1
    ...
    # Create a deep copy of temp_info to work on merging without altering the original.
    temp_info2 = copy.deepcopy(temp_info)
    
    # Compute similarity metrics (representative index, number of significant similarities, p-values,
    # binary matrix, z-score matrix, and correlation matrix) using allmot on the current templates.
    repid_temp, nsig_temp, pval_temp, bmat_temp, zmat_temp, corrmat_temp = allmot(temp_info['template'], nrm)
    
    # Replace NaNs in zmat and bmat with numerical values for further processing.
    zmat_temp_new = np.nan_to_num(zmat_temp, copy=True)
    bmat_temp_new = np.array(np.nan_to_num(bmat_temp, copy=True, nan=0.0))
    
    # For the upper-triangular part of the z-score matrix, compute for each row:
    # (maximum value, row index, index of the maximum value)
    max_dix = np.array([(np.max(x), i, np.argmax(x)) for i, x in enumerate(np.triu(zmat_temp_new))])
    # Sort the rows in descending order based on the maximum z-score
    ...
    # Iterate over each template; here the loop can be thought of as a merging procedure.
    for nt in range(len(temp_info['template'])):
        # Only merge if the binary constraint is satisfied (value == 1) 
        # and the corresponding z-score exceeds the given threshold.
        if (bmat_temp_new[int(mxid[1])][int(mxid[2])] == 1) and (zmat_temp_new[int(mxid[1])][int(mxid[2])] > z_thr):
            # Make a copy of the current cluster assignments.
            ids_clust = np.copy(temp_info2['ids_clust'])
            print("Shape of bmat_temp_new:", np.shape(bmat_temp_new))
            # Find indices where clusters equal either of the two similar clusters.
            cluster_mix_mask = (np.squeeze(temp_info2['ids_clust']) == mxid[2]) | \
                               (np.squeeze(temp_info2['ids_clust']) == mxid[1])
            # Merge the clusters by assigning the merged cluster the value of mxid[2]
            ids_clust[0][cluster_mix_mask] = mxid[2]
            # Reassign unique cluster IDs and update the bad cluster indices.
            _, ids_clust = np.unique(ids_clust[0], return_inverse=True)
            ids_clust[mask_badcluster] = -1

            # Check if any cluster index is missing (due to exclusion criteria) and adjust cluster numbers.
            a = np.arange(0, np.max(ids_clust))
            if len(np.where((np.array([np.sum(x == ids_clust) for x in a]) == 0))[0]):
                b = np.where((np.array([np.sum(x == ids_clust) for x in a]) == 0))[0][0]
                ids_clust = np.where(ids_clust > b, ids_clust - 1, 0)
                ids_clust[mask_badcluster] = -1

            # Delete the old temp_info2 and update it with merged clusters.
            del temp_info2
            temp_info2 = templates(bursts, seqs, nrm, ids_clust, min_ratio=min_ratio)
            # Recompute the similarity metrics for the new templates.
            repid_temp, nsig_temp, pval_temp, bmat_temp, zmat_temp, corrmat_temp = allmot(temp_info2['template'], nrm)
            print('Excluded clusters:', temp_info2['exclude'], 'Mixed clusters:', mxid)

            # Update the new similarity matrices.
            zmat_temp_new = np.nan_to_num(zmat_temp, copy=True)
            bmat_temp_new = np.array(np.nan_to_num(bmat_temp, copy=True, nan=0.0))
            
            # If requested, plot the updated z-score and binary matrices.
            if plot_figure:
                plt.figure()
                plt.subplot(121)
            ...
            # Recompute the maximum similarity index from the updated matrix.
            max_dix = np.array([(np.max(x), i, np.argmax(x)) for i, x in enumerate(np.triu(zmat_temp_new))])
            max_dix_sort = max_dix[np.argsort(np.array(max_dix)[:, 0])[::-1], :]
            mxid = max_dix_sort[0]
            cl_id += 1
...
def allmot(seqs, nrm):
    """
    Compute similarity and significance metrics for a set of sequences.
    
    For each pair of sequences in the input list `seqs`, this function calculates:
      - The rank correlation (using the function `rankseq`)
      - A z-score based on normalization parameters provided in `nrm`
      - A binary indicator (bmat) that flags significant similarity when the z-score exceeds a threshold
    It also computes the number of significant similarities (nsig), corresponding p-values, and a
    representation index (rep_index) for each sequence.
    
    Parameters
    ----------
    seqs : list or array_like
        List of sequences (e.g., arrays of cell indices) whose pairwise similarities are to be computed.
    nrm : array_like
        Normalization parameters, where each row is expected to have at least four values:
          - The first value is used for matching sequence lengths.
          - The second and third values are used to compute the z-score.
          - The fourth value is the threshold for significance.
    
    Returns
    -------
    rep_index : np.ndarray
        Array of representation indices, defined as nsig / std(nsig) for each sequence.
    nsig : np.ndarray
        Array where each element is the sum of significant similarities (binary flags) for a given sequence.
    pval : np.ndarray
        Array of p-values computed from the binomial CDF based on the number of significant similarities.
    bmat : np.ndarray
        Binary matrix (nseqs x nseqs) indicating for each pair whether the z-score exceeds the threshold.
    zmat : np.ndarray
        Matrix (nseqs x nseqs) of z-scores computed from the rank correlations.
    corrmat : np.ndarray
        Matrix (nseqs x nseqs) of rank correlations between sequences.
    """
    nseqs = len(seqs)
    narr = np.array(nrm)[:, 0]
    
    # Initialize matrices for correlation, z-scores, and binary significance
    ...
    # Loop over each pair of sequences (only upper triangular matrix is computed)
    for ns in range(nseqs):
        s1 = seqs[ns]
        zmat[ns, ns] = np.nan  # diagonal elements are not defined
        bmat[ns, ns] = np.nan
        for ms in range(ns+1, nseqs):
            s2 = seqs[ms]
            # Compute rank correlation and overlap length using rankseq (assumed to be defined)
            rc, ln = rankseq(s1, s2)
            ...
            # Compute z-score for the correlation
            ztmp = (rc - mns[1]) / mns[2]
            # Store correlation and z-score symmetrically
            corrmat[ns, ms] = rc
            corrmat[ms, ns] = rc
            zmat[ns, ms] = ztmp
            zmat[ms, ns] = ztmp
            # Set binary flag if the z-score exceeds the threshold (mns[3])
            bmat[ns, ms] = 1. * (ztmp > mns[3])
            bmat[ms, ns] = 1. * (ztmp > mns[3])
        
        # Count the number of significant similarities for sequence ns
        nsig[ns] = np.nansum(bmat[ns, :])
        # Compute a p-value using a binomial cumulative distribution.
        # (Here, nseqs-1 is the number of comparisons, and 0.05 is the chance level.)
        pval[ns] = 1 - binom.cdf(nsig[ns], nseqs - 1, 0.05)
    
    # Compute a representation index for each sequence
...
# print('Clusters removed:', temp_info['exclude'])
# temp_info_test = copy.deepcopy(temp_info)
# temp_info_new, mask = mot.merge_clusters2(temp_info, nrm, seqs, bursts, plot_figure, min_ratio=0.1, z_thr=2.5)
# =============================================================================
...
run_data['seqs'] = [s for s, m in zip(sess_info['seqs'], combined_mask) if m]
