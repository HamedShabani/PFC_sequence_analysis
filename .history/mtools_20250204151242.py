...
def popburstshamed(mat,sig,fs):
    ...
    #idpeaks, _ = find_peaks(poprate, height=thresh,width=(1,spare*10),distance=spare)

    poprate2=np.insert(poprate,0,thresh)# put one value in the beggining to detect peaks in the begginging
    poprate2=np.insert(poprate2,len(poprate2),thresh)# for peaks at the end

    idpeaks, _ = find_peaks(poprate2, height=thresh,width=(1,spare*10),distance=spare)
    
    idpeaks=idpeaks-1# to compaseate shift 
    idpeaks[idpeaks<0]=0

    
    for idpeak in idpeaks:
        if (idpeak>spare) & (mat.shape[1]-idpeak>spare):
            vec.append(mat[:,idpeak-int(spare/2):idpeak+int(spare/2)+1])
       
        elif (idpeak>spare) & (mat.shape[1]-idpeak<=spare):# peaks at the end

            vec.append(mat[:,-spare-1:])

        else:
            vec.append(mat[:,0:spare+1])# peaks in the beggining
...
def binned_burst(dt,winlen,thr_burts,fs,timewins):
    ...
    for twin in np.arange(len(timewins)-1):
        ...
        if np.sum(sdt[:])>-10000:
            #bursts_tmp_s,seqs_tmp_s,ids_temp_s,raster_s,spike_time_s,id_peaks_trl_s,poprate_s = popburstshamed(sdt,thr_burts,fs)
